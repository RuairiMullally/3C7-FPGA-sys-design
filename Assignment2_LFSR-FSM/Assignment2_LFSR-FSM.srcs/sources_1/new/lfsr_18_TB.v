...
// Project Name: 
// Target Devices: lfsr_23b adapted to 18b
// Tool Versions: 
// Description: 
// 
// Dependencies: 
...
module lfsr_18b_TB(
    ...
    wire[17:0] Q_out;
    wire max_tick;
    lfsr_18b uut(.clk(clk), .reset(reset), .shift_enable(shift_enable), .Q_out(Q_out), .max_tick_reg(max_tick)); 
    
    wire[18:0] zero_count, one_count;
    bit_counter bit_count(.clk(clk), .reset(reset), .max_tick(max_tick), .sh_en(shift_enable), .bit_in(Q_out[17]), .zero_count(zero_count), .one_count(one_count));
